...
setValoresIniciais:
	...
	#Seta Valores Iniciais de Pont. I
	li t0, CONST_D0
	li t1, CONST_D1
	li t2, CONST_D2
	li t3, CONST_D3
...
movimenta_princess: 	#os argumentos est�o na mem�ria endereco X(fp)
...
#h� um contador para barris verticais ou horizontais -> se contador%3 == 0 barril vertical, else -> barril horizontal
	lw a0, 12(s0)
	lw a1, 16(s0)
	lw t0, 20(s0)
	lw t1, 24(s0)
	...
	beq	t0, t3, DK_VAI_PARA_BRACO_BAIXO		#se estiver com algum bra�o levantado, o pr�ximo estado � com bra�o abaixado
	beq	t0, t4, DK_VAI_PARA_BRACO_BAIXO		#se estiver com algum bra�o levantado, o pr�ximo estado � com bra�o abaixado
	beq	t0, t5, DX_VAI_PARA_BARRILCENTRO	#se estiver com barril esquerda, o proximo � barril centro
	...
	#significa que est� no estado com bra�o baixo, comparar estado anterior para saber o pr�ximo
		...
		beq t1, t4, DX_VAI_PARA_BARRILESQUERDA	#se no estado anterior ele estava com braco esquerdo, agora pega barril na esquerda
			...
			ret
			 ...
			 sw	t0, 24(s0)
			 sw	s11, 20(s0)
			 #a0 e a1 j� foram carregados no contexto externo
			...
			#faz rem com contador pra ver se vai para direita com barril ou baixar a mao depois de jog�-lo
...
PontuaI:
	...
	#pontua d0: 
	lw a0, 32(s0) #Mudar IMM para adaptar para os outros D's
	la a2, I9
	addi sp, sp, -4
	sw ra, 0(sp)
	jal ra, DesenhaTela
